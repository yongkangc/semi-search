...
#[test]
fn search_filters_by_company_and_topic_metadata() {
    let temp = TempDir::new().expect("temp dir");
    let index_dir = temp.path().join("index");
    let chunks = "tests/fixtures/semiconductor_chunks.jsonl";
    ...
    let results: Value = serde_json::from_slice(&output).expect("search emits JSON results");
    let rows = results.as_array().expect("array results");
    assert!(!rows.is_empty(), "expected filtered search hit");
    let serialized = serde_json::to_string(&results).expect("serialize results");
    assert!(
        serialized.contains("TSMC CoWoS"),
        "expected TSMC CoWoS result: {serialized}"
    );
    assert!(
        serialized.contains("advanced-packaging"),
        "expected topic metadata: {serialized}"
    );
}
